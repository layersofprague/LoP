...
function _html() {
  ...
  return items.map(item => `
    <a class="lop-n-item${item.id === active ? ' active' : ''}" href="${item.href}">
      ...
      ${item.locked ? `<span class="lop-n-badge-lock"><svg width="6" height="6" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></span>` : ''}
    </a>
  `).join('');
}
